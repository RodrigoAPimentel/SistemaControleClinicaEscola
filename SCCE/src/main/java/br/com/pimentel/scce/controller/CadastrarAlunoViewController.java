 ...
 * SCCE
 * 
 * Classe de Controller de Cadastrar Aluno
 */
public class CadastrarAlunoViewController implements Initializable, Serializable{

	private static final long serialVersionUID = 533698194976927058L;
	
	private static Logger logger = LoggerFactory.getLogger(CadastrarAlunoViewController.class);
    ...
    @FXML private JFXTextField textFieldMatricula;
    ...
    @FXML void clickedCadastrar(MouseEvent event) {
    	...
    	if (textFieldCPF.getText().length()>0 && textFieldNome.getText().length()>0 && textFieldCEP.getText().length()>0 
    		...
    		&& textFieldCurso.getText().length()>0 && textFieldMatricula.getText().length()>0 && comboBoxAreaMedica.getValue() != null) {
    		...
    		if (alunoService.buscarPorCPF(textFieldCPF.getText()) == null) {
    			try {
        		...
        		aluno = new Aluno(textFieldCPF.getText(), textFieldNome.getText(), datePickerNascimento.getValue(), endereco, textFieldMatricula.getText(), 
        		...
        		try {
    				...
    				logger.info("+++ ALUNO DE CPF: [{}] CADASTRADO", textFieldCPF.getText()); 
    			} catch (Exception e) {
    				logger.info(e.getMessage());
    			}
    			...
    			dialogoConfirmacao.setHeaderText("ALUNO CADASTRADO");
				...
				logger.error("+++ ALUNO DE CPF: [{}] JA ESTA CADASTRADO", textFieldCPF.getText());
    			
    			dialogoError.setHeaderText("ALUNO JA ESTA CADASTRADO");
    			dialogoError.setContentText("Ja existe um Aluno com CPF \"" + textFieldCPF.getText() + "\" cadastrado");
    			dialogoError.showAndWait();
			}
		}else {
			dialogoAviso.setHeaderText("CAMPO(S) NÃO PREENCHIDO(S)");
	        dialogoAviso.setContentText("Você precisa preencher todos os campo para Cadastrar um Aluno.");
	        dialogoAviso.showAndWait();
		}    	
    }

    @FXML void clickedSair(MouseEvent event) {
    	logger.info("+++ FECHANDO JANELA CADASTRAR ALUNO");
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		logger.info("+++ INICIANDO ADICIONAR ALUNO CONTROLLER");
	...
	private void aplicarMascaras() {
		...
		MascarasFX.numericField(textFieldMatricula);
	}
